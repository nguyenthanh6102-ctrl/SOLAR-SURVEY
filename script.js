      ...
      img.onload = () => {
        ...
        itemDiv.innerHTML = `
          <div class="markup-toolbar no-print">
            ...
            <button type="button" class="tool-btn" onclick="clearCanvas('${uniqueId}')">🧹 Xóa</button>
...
function setupDrawing(id) {
  ...
  const start = (e) => {
    ...
    if (s.tool === 'text') {
      ...
      if (text) {
        s.ctx.fillStyle = "red";
        s.ctx.font = "bold " + (cv.width * 0.04) + "px Arial";
  ...
  const move = (e) => {
    if (!s.isDrawing || s.tool === 'text') return;
    const p = getPos(e);
...
function deleteImage(id) {
  if(confirm("Xóa ảnh này?")) { document.getElementById("item_" + id).remove(); delete canvasStates[id]; }
}
...
async function exportPDF() {
  ...
  try {
    ...
    pdf.save(`GP_Solar_Survey_${Date.now()}.pdf`);
...
function exportData() {
  ...
  saveAs(new Blob([JSON.stringify(data)], {type: "application/json"}), `SurveyData_${Date.now()}.json`);
}
...
function handlePreviewFromData(categoryName, area, img, idx) {
    ...
    itemDiv.id = "item_" + uniqueId;
    itemDiv.className = "markup-container";
    itemDiv.innerHTML = `<div class="markup-toolbar no-print"><button type="button" class="tool-btn active" onclick="setTool('${uniqueId}','pen',this)">🖊️</button><button type="button" class="tool-btn" onclick="setTool('${uniqueId}','text',this)">🔤</button><button type="button" class="tool-btn" onclick="clearCanvas('${uniqueId}')">🧹</button><button type="button" class="tool-btn btn-del-img" onclick="deleteImage('${uniqueId}')">❌</button></div><div class="canvas-wrapper"><canvas id="canvas_${uniqueId}"></canvas></div>`;
    area.appendChild(itemDiv);
    const canvas = document.getElementById(`canvas_${uniqueId}`);
    const ctx = canvas.getContext('2d');
